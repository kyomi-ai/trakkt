...
//! `trakkt_core::Result<T>`. KV, config, and encryption key args follow.
...
pub async fn login_with_password_service(
...
) -> trakkt_core::Result<LoginServiceResult> {
    ...
    let hash = password_method
        ...
        .get("hash")
        .and_then(|v| v.as_str())
        .ok_or_else(|| trakkt_core::Error::Internal("Password auth method missing hash".into()))?;

    // Verify password
    if !crate::password::verify_password(password, hash)
        .map_err(|e| trakkt_core::Error::Internal(format!("Password verification error: {e}")))?
    {
...
pub struct SignupStartParams<'a> {
    ...
    pub config: Option<&'a trakkt_core::Config>,
}
...
) -> trakkt_core::Result<SignupStartServiceResult> {
...
struct SmtpLessNewUserParams<'a> {
    ...
    device: &'a DeviceInfo,
    config: Option<&'a trakkt_core::Config>,
}
...
async fn signup_smtp_less_new_user(
    ...
    let hash = crate::password::hash_password(password_str)
        .map_err(|e| trakkt_core::Error::Internal(format!("Failed to hash password: {e}")))?;
    ...
    let user = crate::user_service::get_user_by_email(db, email)
        .await?
        .ok_or_else(|| trakkt_core::Error::Internal("User not found after creation".into()))?;
...
async fn signup_smtp_less_existing_unverified(
    ...
    let user = crate::user_service::get_user_by_email(db, email)
        .await?
        .ok_or_else(|| trakkt_core::Error::Internal("User not found after signup".into()))?;
...
pub async fn signup_complete_service(
...
) -> trakkt_core::Result<SignupCompleteServiceResult> {
    ...
    let user = crate::user_service::get_user_by_email(db, &email)
        .await?
        .ok_or_else(|| trakkt_core::Error::Internal("User not found for verified token".into()))?;
    ...
    let user = crate::user_service::get_user_by_email(db, &email)
        .await?
        .ok_or_else(|| {
            trakkt_core::Error::Internal("User not found after signup completion".into())
        })?;
...
) -> trakkt_core::Result<GoogleOAuthServiceResult> {
...
pub async fn recovery_verify_service(
    ...
    kv: &KVPool,
    token: &str,
) -> trakkt_core::Result<RecoveryVerifyServiceResult> {
    let email =
    ...
    let user = crate::user_service::get_user_by_email(db, &email)
        .await?
        .ok_or_else(|| trakkt_core::Error::Internal("User not found for recovery token".into()))?;
...
pub async fn recovery_set_password_service(
    ...
    new_password: &str,
    device: &DeviceInfo,
) -> trakkt_core::Result<RecoverySetPasswordServiceResult> {
    ...
    let user = crate::user_service::get_user_by_id(db, &user_id)
        .await?
        .ok_or_else(|| {
            trakkt_core::Error::Internal("User not found for recovery session".into())
        })?;
...
pub async fn passkey_login_complete_service(
...
) -> trakkt_core::Result<PasskeyLoginServiceResult> {
    ...
    let credential: PublicKeyCredential = serde_json::from_str(assertion_json)
        .map_err(|e| trakkt_core::Error::Internal(format!("Invalid assertion JSON: {e}")))?;
    ...
    if is_discoverable {
        ...
        .map_err(|e| {
            ...
            trakkt_core::Error::Internal("Authentication failed".into())
        });
        match auth_result {
            Ok(auth_result) => {
        ...
        let auth_state: PasskeyAuthentication =
            serde_json::from_value(challenge_data["authentication_state"].clone())
                .map_err(|e| {
                    trakkt_core::Error::Internal(format!("Deserialize auth state: {e}"))
                })?;
...
pub async fn passkey_register_start_service(
...
) -> trakkt_core::Result<PasskeyRegisterStartServiceResult> {
    ...
    let (ccr, reg_state) =
        crate::webauthn::start_registration(webauthn, user_unique_id, email, display_name, exclude_opt)
            .map_err(|e| trakkt_core::Error::Internal(e.to_string()))?;
    ...
    let creation_challenge = serde_json::to_string(&ccr)
        .map_err(|e| trakkt_core::Error::Internal(format!("Serialize creation challenge: {e}")))?;
...
pub async fn passkey_register_complete_service(
    ...
    credential_json: &str,
    device: &DeviceInfo,
) -> trakkt_core::Result<AuthenticatedSession> {
    use base64::{Engine as _, engine::general_purpose::URL_SAFE_NO_PAD};
    use webauthn_rs::prelude::*;

    let credential: RegisterPublicKeyCredential =
        serde_json::from_str(credential_json)
            .map_err(|e| trakkt_core::Error::Internal(format!("Invalid credential JSON: {e}")))?;

    // Get and delete challenge
    let challenge_data = crate::redis_ops::get_webauthn_challenge(kv, challenge_id)
        .await?
        .ok_or_else(|| {
            trakkt_core::Error::Internal("Invalid or expired challenge".into())
        })?;
    crate::redis_ops::delete_webauthn_challenge(kv, challenge_id).await?;
    ...
    let email = challenge_data["email"]
        .as_str()
        .ok_or_else(|| trakkt_core::Error::Internal("Missing email in challenge".into()))?;
    let user_id = challenge_data["user_id"]
        .as_str()
        .ok_or_else(|| trakkt_core::Error::Internal("Missing user_id in challenge".into()))?;
...
) -> trakkt_core::Result<Result<(String, String), String>> {
...
pub async fn passkey_recovery_verify_service(
    ...
    webauthn: &webauthn_rs::Webauthn,
    token: &str,
) -> trakkt_core::Result<Result<(String, String, String), String>> {
...
pub async fn get_passkeys_for_user(
    db: &DbPool,
    user_id: &str,
) -> trakkt_core::Result<Vec<webauthn_rs::prelude::Passkey>> {
...
pub async fn resend_verification_service(
    ...
    ip: &str,
    email: &str,
) -> trakkt_core::Result<Option<ResendVerificationResult>> {
...
pub async fn recovery_start_service(
    ...
    ip: &str,
    email: &str,
) -> trakkt_core::Result<Option<RecoveryStartResult>> {
    let rate = crate::rate_limiter::check_rate_limit(kv, ip, "register", None).await?;
    if !rate.allowed {
