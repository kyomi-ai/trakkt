...
/// Content-ID used in `<img src="cid:trakkt_logo">`.
const LOGO_CID: &str = "trakkt_logo";
...
pub struct EmailService {
    ...
    from_email: String,
    from_name: String,
    /// Base URL for the frontend app (e.g. `https://app.trakkt.app`).
    frontend_url: String,
}
...
impl EmailService {
    ...
    pub fn from_env() -> Self {
        ...
        let from_email =
            std::env::var("SMTP_FROM_EMAIL").unwrap_or_else(|_| "noreply@trakkt.app".to_string());
        let from_name =
            std::env::var("SMTP_FROM_NAME").unwrap_or_else(|_| "Trakkt".to_string());
        let frontend_url = std::env::var("FRONTEND_URL")
            .unwrap_or_else(|_| "https://app.trakkt.app".to_string())
            .trim_end_matches('/')
            .to_string();
    ...
    pub async fn send_email(
        ...
        let send_result = trakkt_core::retry::retry_with_backoff_classified(
            || {
    ...
    pub async fn send_workspace_invitation(
        ...
        let subject = format!("You've been invited to join {} on Trakkt", workspace_name);
        ...
        let footer_html = format!(
            ...
            You're receiving this because you were invited to join a workspace on Trakkt.
        </p>
        <p style="margin: 0;">
            ...
            <a href="{frontend_url}" style="color: #4f46e5;">trakkt.app</a>
        </p>"#
        );
...
"#,
    ...
    pub async fn send_passkey_recovery(
        ...
        let footer_html = format!(
            r#"<p style="margin: 0 0 8px 0;">
            You're receiving this because you requested account recovery for Trakkt.
        </p>
        <p style="margin: 0;">
            <a href="{escaped_frontend}" style="color: #4f46e5;">trakkt.app</a>
        </p>"#
        );
...
---
You're receiving this email because you requested account recovery for Trakkt.
{frontend_url}
",
        );
...
",
    ...
    pub async fn send_verification_email(
        ...
        <p>Thanks for signing up for Trakkt! Click the button below to verify your email address and complete your account setup:</p>
        ...
        <p>If you didn't create a Trakkt account, please ignore this email.</p>
        ...
        let footer_html = format!(
            r#"<p style="margin: 0 0 8px 0;">
            You're receiving this because someone signed up for Trakkt with this email address.
        </p>
        <p style="margin: 0;">
            <a href="{escaped_frontend}" style="color: #4f46e5;">trakkt.app</a>
        </p>"#
...
Thanks for signing up for Trakkt! Click the link below to verify your email address and complete your account setup:

{verification_link}

IMPORTANT: This link expires in 24 hours.

If you didn't create a Trakkt account, please ignore this email.
...
",
    ...
    pub async fn send_subscription_welcome(
        ...
        let frontend_url = &self.frontend_url;
        let subject = "Welcome to Trakkt!";

        let escaped_email = html_escape(email);

        let body_html = format!(
            r#"<h1>Welcome to Trakkt!</h1>
...
/// Produce a full HTML email document with the shared Trakkt branding.
///
...
<body style="background-color: #FAFAF8; color: #1C1917;">
    <div class="header">
        <a href="{frontend_url}" style="text-decoration: none;">
            <img src="cid:trakkt_logo" alt="Trakkt" class="logo-img" style="height: 48px; width: auto;">
        </a>
    </div>
    <div class="content">
        {body_html}
...
<body>
    <div class="header">
        <a href="{frontend_url}" style="text-decoration: none;">
            <img src="cid:trakkt_logo" alt="Trakkt" style="height: 48px; width: auto;">
        </a>
    </div>
    ...
    <div class="footer">
        <p style="margin:0;"><a href="{frontend_url}" style="color: #4f46e5;">trakkt.app</a></p>
    </div>
</body>
</html>"#
    )
...
mod tests {
    ...
    fn email_service_not_configured_by_default() {
        ...
        let service = EmailService {
            ...
            smtp_user: None,
            smtp_password: None,
            from_email: "noreply@trakkt.app".to_string(),
            from_name: "Trakkt".to_string(),
            frontend_url: "https://app.trakkt.app".to_string(),
        };
