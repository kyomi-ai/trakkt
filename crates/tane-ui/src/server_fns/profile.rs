...
pub async fn get_pending_invitations() -> Result<Vec<InvitationData>, ServerFnError> {
    ...
    let invitations =
        tane_auth::workspace_service::get_pending_invitations_for_email(&ctx.db, &auth.email)
            .await
            .into_sfn()?;
...
pub async fn update_profile_name(name: String) -> Result<(), ServerFnError> {
    ...
    tane_auth::user_service::update_user_name(&ctx.db, &auth.user_id, &trimmed)
        .await
        .into_sfn()?;
...
pub async fn update_theme(theme: String) -> Result<(), ServerFnError> {
    ...
    tane_auth::user_service::update_extra_metadata(&ctx.db, &auth.user_id, &metadata)
        .await
        .into_sfn()?;
...
pub async fn update_default_dashboard(dashboard_id: Option<String>) -> Result<(), ServerFnError> {
    ...
    tane_auth::user_service::update_extra_metadata(&ctx.db, &auth.user_id, &value)
        .await
        .into_sfn()?;
...
pub async fn accept_invitation(invitation_id: String) -> Result<(), ServerFnError> {
    ...
    tane_auth::workspace_service::accept_invitation_for_user(
        &ctx.db,
        &invitation_id,
        &auth.user_id,
    )
...
pub async fn decline_invitation(invitation_id: String) -> Result<(), ServerFnError> {
    ...
    tane_auth::workspace_service::update_invitation_status(
        &ctx.db,
        &invitation_id,
        "declined",
    )
