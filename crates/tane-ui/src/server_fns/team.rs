...
fn map_role_to_db(role: &str) -> &'static str {
    let roles = &tane_core::constants::get().workspace.roles;
    match role {
        "admin" => &roles.admin,
        _ => &roles.user,
    }
...
async fn get_current_workspace(
    db: &tane_core::DbPool,
    ws_id: &str,
) -> Result<tane_core::models::Workspace, ServerFnError> {
    tane_auth::workspace_service::get_workspace_full(db, ws_id)
        .await
        .into_sfn()?
...
pub async fn list_workspace_members() -> Result<Vec<TeamMember>, ServerFnError> {
    ...
    let members =
        tane_auth::workspace_service::get_workspace_members_with_users(ac.db(), &ac.ws_id)
            .await
            .into_sfn()?;
...
pub async fn update_member_role(user_id: String, role: String) -> Result<(), ServerFnError> {
    ...
    if user_id == ac.auth.user_id && db_role == "workspace_user" {
        let admin_count =
            tane_auth::workspace_service::count_admins(ac.db(), &ac.ws_id)
                .await
                .into_sfn()?;
        if admin_count < 2 {
    ...
    tane_auth::workspace_service::update_member_role(ac.db(), &ac.ws_id, &user_id, db_role)
        .await
        .into_sfn()?;
...
pub async fn remove_member(user_id: String) -> Result<(), ServerFnError> {
    ...
    tane_auth::workspace_service::remove_workspace_member(
        ac.db(),
        &ac.ws_id,
...
pub async fn list_workspace_invitations() -> Result<Vec<TeamInvitation>, ServerFnError> {
    ...
    let invitations =
        tane_auth::workspace_service::get_pending_invitations_for_workspace(ac.db(), &ac.ws_id)
            .await
            .into_sfn()?;
...
pub async fn invite_member(email: String, role: String) -> Result<(), ServerFnError> {
    ...
    tane_auth::workspace_service::invite_workspace_member(
        tane_auth::workspace_service::InviteWorkspaceMemberParams {
            pool: ac.db(),
...
pub async fn cancel_invitation(invitation_id: String) -> Result<(), ServerFnError> {
    ...
    let invitation =
        tane_auth::workspace_service::get_invitation_in_workspace(ac.db(), &invitation_id, &ac.ws_id)
            .await
            .into_sfn()?
            .ok_or_else(|| ServerFnError::new("Invitation not found"))?;

    if invitation.status != tane_core::enums::InvitationStatus::Pending {
        return Err(ServerFnError::new("Can only cancel pending invitations"));
    }

    tane_auth::workspace_service::update_invitation_status(ac.db(), &invitation_id, "cancelled")
        .await
        .into_sfn()?;
...
pub async fn list_ownership_transfers() -> Result<Vec<OwnershipTransferData>, ServerFnError> {
    ...
    let transfers =
        tane_auth::workspace_service::list_ownership_transfers_for_user(ac.db(), &ac.ws_id, &ac.auth.user_id)
            .await
            .into_sfn()?;
...
pub async fn cancel_ownership_transfer(transfer_id: String) -> Result<(), ServerFnError> {
    ...
    let transfer =
        tane_auth::workspace_service::get_ownership_transfer(&ctx.db, &transfer_id)
            .await
            .into_sfn()?
    ...
    tane_auth::workspace_service::update_transfer_status(&ctx.db, &transfer_id, "cancelled")
        .await
        .into_sfn()?;
...
pub async fn initiate_ownership_transfer(to_user_id: String) -> Result<(), ServerFnError> {
    ...
    let existing =
        tane_auth::workspace_service::get_pending_transfer_for_workspace(ac.db(), &ac.ws_id)
            .await
            .into_sfn()?;
    if existing.is_some() {
        return Err(ServerFnError::new(
    ...
    tane_auth::workspace_service::create_ownership_transfer(
        ac.db(),
        &transfer_id,
        &ac.ws_id,
        &ac.auth.user_id,
    ...
    let to_user = tane_auth::user_service::get_user_by_id(ac.db(), &to_user_id)
        .await
        .into_sfn()?;
    if let Some(to_user) = to_user {
        ...
        tokio::spawn(async move {
            let svc = tane_auth::email_service::EmailService::from_env();
