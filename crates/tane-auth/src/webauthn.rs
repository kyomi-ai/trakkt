...
pub fn build_webauthn(rp_id: &str, rp_name: &str, rp_origin: &Url) -> tane_core::Result<Webauthn> {
    let builder = WebauthnBuilder::new(rp_id, rp_origin)
        .map_err(|e| tane_core::Error::Internal(format!("WebAuthn builder error: {e}")))?
        .rp_name(rp_name);

    builder
        .build()
        .map_err(|e| tane_core::Error::Internal(format!("WebAuthn build error: {e}")))
}
...
pub fn start_registration(
...
) -> tane_core::Result<(CreationChallengeResponse, PasskeyRegistration)> {
    let (mut ccr, reg_state) = webauthn
        ...
        .map_err(|e| tane_core::Error::Internal(format!("WebAuthn registration start: {e}")))?;
...
) -> tane_core::Result<Passkey> {
    webauthn
        .finish_passkey_registration(credential, registration_state)
        .map_err(|e| tane_core::Error::BadRequest(format!("WebAuthn registration failed: {e}")))
}
...
pub fn start_authentication(
    webauthn: &Webauthn,
    credentials: &[Passkey],
) -> tane_core::Result<(RequestChallengeResponse, PasskeyAuthentication)> {
    webauthn
        .start_passkey_authentication(credentials)
        .map_err(|e| tane_core::Error::Internal(format!("WebAuthn authentication start: {e}")))
}
...
) -> tane_core::Result<AuthenticationResult> {
    webauthn
        .finish_passkey_authentication(credential, authentication_state)
        .map_err(|e| tane_core::Error::BadRequest(format!("WebAuthn authentication failed: {e}")))
}
...
pub fn start_discoverable_authentication(
    webauthn: &Webauthn,
) -> tane_core::Result<(RequestChallengeResponse, DiscoverableAuthentication)> {
    webauthn
        .start_discoverable_authentication()
        .map_err(|e| tane_core::Error::Internal(format!("WebAuthn discoverable auth start: {e}")))
}
...
pub fn finish_discoverable_authentication(
    ...
    webauthn
        ...
        .map_err(|e| tane_core::Error::BadRequest(format!("WebAuthn discoverable auth failed: {e}")))
}
