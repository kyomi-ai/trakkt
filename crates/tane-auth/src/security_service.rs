...
//! inlined in `tane-ui/src/server_fns/security.rs`, leaving those server
//! functions as thin wrappers.
//!
//! All functions take `&DbPool` as their first argument and return
//! `tane_core::Result<T>`. Functions that need the KV store also accept
//! `&tane_core::KVPool`. Functions that need WebAuthn accept
...
pub async fn change_password(
    ...
    let Some(password_method) = password_method else {
        return Err(tane_core::Error::Internal(
...
pub async fn setup_totp(
    ...
    user_id: &str,
    email: &str,
) -> tane_core::Result<TotpSetupResult> {
    let enabled = crate::user_service::has_totp_enabled(pool, user_id).await?;
    if enabled {
        return Err(tane_core::Error::Internal("2FA is already enabled".into()));
    }
...
) -> tane_core::Result<(Vec<crate::token_service::SessionInfo>, Option<String>)> {
...
pub async fn logout(pool: &DbPool, raw_refresh_token: Option<&str>) -> tane_core::Result<()> {
...
pub async fn start_passkey_registration(
    ...
    user_id: &str,
    device_name: &str,
) -> tane_core::Result<(CreationChallengeResponse, String)> {
...
pub async fn complete_passkey_registration(
    ...
    let challenge_data = crate::redis_ops::get_webauthn_challenge(kv, challenge_id)
        .await?
        .ok_or_else(|| tane_core::Error::Internal("Invalid or expired challenge".into()))?;
