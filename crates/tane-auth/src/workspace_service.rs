...
use tane_types::sync::{SyncActionType, entity_types};
...
pub async fn get_workspace_users(
    pool: &DbPool,
    workspace_id: &str,
) -> tane_core::Result<Vec<WorkspaceUser>> {
    ...
    let users = tane_core::db_fetch_all!(pool, WorkspaceUser, &sql, workspace_id)?;
    Ok(users)
}
...
pub async fn count_workspace_users(
    ...
    let count: i64 = tane_core::db_fetch_scalar!(pool, i64, &sql, workspace_id)?;
    Ok(count)
}
...
pub async fn get_user_workspaces(
    pool: &DbPool,
    user_id: &str,
) -> tane_core::Result<Vec<(Workspace, WorkspaceUser)>> {
    let is_pg = pool.is_postgres();
    let bt = sql_compat::bool_true(is_pg);
    ...
    let memberships = tane_core::db_fetch_all!(pool, WorkspaceUser, &memberships_sql, user_id)?;
...
pub async fn get_workspace_settings_for_sync(
    ...
    let row = tane_core::db_fetch_optional!(
        pool,
...
pub async fn update_workspace_name(
    ...
    let result = tane_core::db_execute!(pool, &sql, name, workspace_id)?;
...
pub async fn update_workspace_settings(
    ...
    let result = tane_core::db_execute!(pool, &sql, &settings_str, workspace_id)?;
...
pub async fn get_workspace_members_with_users(
    pool: &DbPool,
    workspace_id: &str,
) -> tane_core::Result<Vec<MemberWithUser>> {
    let is_pg = pool.is_postgres();
    let bt = sql_compat::bool_true(is_pg);
    let sql = format!(
...
pub async fn update_member_role(
    ...
    let result = tane_core::db_execute!(pool, &sql, new_role, workspace_id, user_id)?;
    Ok(result.rows_affected() > 0)
}
...
pub async fn create_workspace_user(
    ...
    tane_core::db_execute!(pool, &sql, workspace_id, user_id, role)?;
    Ok(())
}
...
pub async fn create_invitation(
...
) -> tane_core::Result<WorkspaceInvitation> {
    ...
    get_invitation(pool, invitation_id)
        .await?
        .ok_or_else(|| tane_core::Error::Internal("Invitation created but not found".into()))
}
...
pub async fn get_invitation(
    pool: &DbPool,
    invitation_id: &str,
) -> tane_core::Result<Option<WorkspaceInvitation>> {
    let inv = tane_core::db_fetch_optional!(
...
pub async fn get_pending_invitations_for_workspace(
    pool: &DbPool,
    workspace_id: &str,
) -> tane_core::Result<Vec<WorkspaceInvitation>> {
    let invitations = tane_core::db_fetch_all!(
        pool, WorkspaceInvitation,
        "SELECT * FROM workspace_invitations \
...
pub async fn count_pending_invitations(
    ...
    let count: i64 = tane_core::db_fetch_scalar!(
        pool, i64,
...
pub async fn check_existing_member_by_email(
    ...
    let count: i64 = tane_core::db_fetch_scalar!(pool, i64, &sql, workspace_id, email)?;
    Ok(count > 0)
}
...
pub async fn accept_invitation(
    ...
    let result = tane_core::db_execute!(pool, &sql, user_id, invitation_id)?;
    Ok(result.rows_affected() > 0)
}
...
pub async fn accept_invitation_for_user(
    ...
    let invitation = get_invitation(pool, invitation_id)
        .await?
        .ok_or_else(|| tane_core::Error::NotFound("Invitation not found".into()))?;
...
pub async fn create_ownership_transfer(
    ...
    to_user_id: &str,
    expires_at: chrono::DateTime<chrono::Utc>,
) -> tane_core::Result<OwnershipTransfer> {
    tane_core::db_execute!(
        pool,
    ...
    get_ownership_transfer(pool, transfer_id)
        .await?
        .ok_or_else(|| tane_core::Error::Internal("Transfer created but not found".into()))
}
...
pub async fn get_ownership_transfer(
    pool: &DbPool,
    transfer_id: &str,
) -> tane_core::Result<Option<OwnershipTransfer>> {
    let transfer = tane_core::db_fetch_optional!(
...
pub async fn get_pending_transfers_for_user(
    pool: &DbPool,
    user_id: &str,
) -> tane_core::Result<Vec<OwnershipTransfer>> {
    let transfers = tane_core::db_fetch_all!(
        pool, OwnershipTransfer,
        "SELECT * FROM ownership_transfers \
...
pub async fn update_transfer_status(
    ...
    let result = tane_core::db_execute!(pool, &sql, status, transfer_id)?;
    Ok(result.rows_affected() > 0)
}
...
pub async fn complete_ownership_transfer(
    ...
    match pool {
        tane_core::db::DbPool::Postgres(pg) => {
...
pub async fn update_workspace_owner(
    ...
    let result = tane_core::db_execute!(pool, &sql, new_owner_id, workspace_id)?;
    Ok(result.rows_affected() > 0)
}
...
pub async fn get_transfer_for_recipient(
    ...
    transfer_id: &str,
    recipient_id: &str,
) -> tane_core::Result<Option<OwnershipTransferDetail>> {
...
pub async fn remove_workspace_member(
    ...
    if target_user_id == owner_user_id {
        return Err(tane_core::Error::BadRequest(
...
pub async fn list_ownership_transfers_for_user(
    ...
    workspace_id: &str,
    user_id: &str,
) -> tane_core::Result<Vec<ResolvedTransfer>> {
