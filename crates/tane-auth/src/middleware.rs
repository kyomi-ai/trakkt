...
use tane_core::enums::{WorkspaceRole, WorkspaceStatus};
...
where
    ...
    AuthState: FromRef<S>,
{
    type Rejection = tane_core::Error;
    ...
    async fn from_request_parts(parts: &mut Parts, state: &S) -> Result<Self, Self::Rejection> {
        ...
        let user = crate::user_service::get_user_by_id(&auth_state.db, &user_id)
            ...
            .map_err(|e| {
                ...
                tane_core::Error::Internal("database error".into())
            })?
            .ok_or_else(|| tane_core::Error::Unauthorized("User not found".into()))?;

        if !user.active {
            return Err(tane_core::Error::Unauthorized("User account is inactive".into()));
        }
                ...
                Ok(Some(ws)) => {
                    match crate::user_service::get_workspace_user(&auth_state.db, ws_id, &user_id).await {
                        ...
                        Ok(None) => {
                            ...
                            return Err(tane_core::Error::Unauthorized(
...
fn extract_token(parts: &Parts) -> tane_core::Result<String> {
    // Check Authorization: Bearer <token>
    if let Some(auth_header) = parts.headers.get("authorization") {
        let value = auth_header
            .to_str()
            .map_err(|_| tane_core::Error::Unauthorized("invalid auth header".into()))?;
    ...
    let cookie_name = &tane_core::constants::get().cookies.access_token_name;
    let cookie_prefix = format!("{cookie_name}=");
    if let Some(cookie_header) = parts.headers.get("cookie") {
        let cookies = cookie_header
            .to_str()
            .map_err(|_| tane_core::Error::Unauthorized("invalid cookie header".into()))?;
    ...
    Err(tane_core::Error::Unauthorized(
...
async fn load_personal_user(db: &tane_core::DbPool) -> tane_core::Result<AuthUser> {
    let user = crate::user_service::get_user_by_id(db, "user-local")
        ...
        .ok_or_else(|| {
            ...
            tane_core::Error::ServiceUnavailable(
