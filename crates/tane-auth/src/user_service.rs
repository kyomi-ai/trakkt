...
use tane_core::models::{ApiToken, User, UserAuthMethod, Workspace, WorkspaceUser};
use tane_core::sql_compat;
use tane_core::DbPool;
use rand::Rng;
use sha2::{Digest, Sha256};

/// Returns true if any users exist in the database.
pub async fn has_any_users(pool: &DbPool) -> tane_core::Result<bool> {
    // Use EXISTS for efficiency — stops scanning after first row
    let row: (i32,) = tane_core::db_fetch_one!(
        pool, (i32,),
...
pub async fn get_user_by_id(pool: &DbPool, user_id: &str) -> tane_core::Result<Option<User>> {
    let user = tane_core::db_fetch_optional!(
        pool, User,
...
pub async fn get_user_by_email(pool: &DbPool, email: &str) -> tane_core::Result<Option<User>> {
    let user = tane_core::db_fetch_optional!(
        pool, User,
...
pub async fn update_last_login(pool: &DbPool, user_id: &str) -> tane_core::Result<bool> {
    let is_pg = pool.is_postgres();
    let now = sql_compat::now(is_pg);
    let sql = format!(
        "UPDATE users SET last_login = {now}, updated_at = {now} WHERE user_id = $1"
    );
    let result = tane_core::db_execute!(pool, &sql, user_id)?;
...
pub async fn update_user_name(
    ...
    user_id: &str,
    name: &str,
) -> tane_core::Result<bool> {
    let is_pg = pool.is_postgres();
    let now = sql_compat::now(is_pg);
    let sql = format!(
        "UPDATE users SET name = $1, updated_at = {now} WHERE user_id = $2"
    );
    let result = tane_core::db_execute!(pool, &sql, name, user_id)?;
    Ok(result.rows_affected() > 0)
}
...
pub async fn update_last_workspace(
    ...
    let result = tane_core::db_execute!(pool, &sql, workspace_id, user_id)?;
    Ok(result.rows_affected() > 0)
}

/// Mark a user as email-verified.
pub async fn mark_user_verified(pool: &DbPool, email: &str) -> tane_core::Result<bool> {
    let is_pg = pool.is_postgres();
    let now = sql_compat::now(is_pg);
    ...
    let result = tane_core::db_execute!(pool, &sql, email)?;
    Ok(result.rows_affected() > 0)
}

/// Check whether a user has a password auth method.
pub async fn has_password(pool: &DbPool, user_id: &str) -> tane_core::Result<bool> {
    let is_pg = pool.is_postgres();
    let bt = sql_compat::bool_true(is_pg);
    let sql = format!(
    ...
    let count: i64 = tane_core::db_fetch_scalar!(pool, i64, &sql, user_id)?;
    Ok(count > 0)
}
...
pub async fn get_workspace(
    pool: &DbPool,
    workspace_id: &str,
) -> tane_core::Result<Option<Workspace>> {
    let ws = tane_core::db_fetch_optional!(
        pool, Workspace,
...
pub async fn get_user_workspace_context(
    pool: &DbPool,
    user_id: &str,
) -> tane_core::Result<Option<(Workspace, WorkspaceUser)>> {
    ...
    let wu = tane_core::db_fetch_optional!(pool, WorkspaceUser, &sql, user_id)?;
...
pub async fn create_user(
    ...
    tane_core::db_execute!(pool, &sql, &user_id, email, display_name, verified)?;

    // Return the created user
    get_user_by_id(pool, &user_id)
        .await?
        .ok_or_else(|| tane_core::Error::Internal("User created but not found".into()))
}
...
pub async fn create_workspace_for_user(
    ...
    tane_core::db_execute!(pool, &ws_user_sql, &workspace_id, user_id)?;
...
pub async fn update_user_oauth_data(
    ...
    let result = tane_core::db_execute!(pool, &sql, encrypted_data, user_id)?;
    Ok(result.rows_affected() > 0)
}
...
pub async fn update_terms_acceptance(
    ...
    let result = tane_core::db_execute!(pool, &sql, terms_version, marketing_consent, user_id)?;
    Ok(result.rows_affected() > 0)
}
...
pub async fn get_auth_method(
    ...
    user_id: &str,
    auth_type: &str,
) -> tane_core::Result<Option<UserAuthMethod>> {
    let is_pg = pool.is_postgres();
    let bt = sql_compat::bool_true(is_pg);
    let sql = format!(
    ...
    let method = tane_core::db_fetch_optional!(pool, UserAuthMethod, &sql, user_id, auth_type)?;
    Ok(method)
}
    ...
    match pool {
        DbPool::Postgres(pg) => {
            sqlx::query(&sql)
                ...
                .execute(pg)
                .await
                .map_err(|e| tane_core::Error::Internal(format!("upsert_auth_method: {e}")))?;
        }
        DbPool::Sqlite(sq) => {
            let auth_data_str = serde_json::to_string(auth_data)
                .map_err(|e| tane_core::Error::Internal(format!("JSON serialization failed: {e}")))?;
            sqlx::query(&sql)
                .bind(user_id)
                .bind(auth_type)
                .bind(&auth_data_str)
...
pub async fn touch_auth_method(
    ...
    tane_core::db_execute!(pool, &sql, user_id, auth_type)?;
    Ok(())
}

/// Check if a user has TOTP (2FA) enabled.
pub async fn has_totp_enabled(pool: &DbPool, user_id: &str) -> tane_core::Result<bool> {
    let is_pg = pool.is_postgres();
    let bt = sql_compat::bool_true(is_pg);
    let sql = format!(
        "SELECT COUNT(*) as count FROM user_auth_methods \
...
pub async fn remove_auth_method(
    ...
    let result = tane_core::db_execute!(pool, &sql, user_id, auth_type)?;
    Ok(result.rows_affected() > 0)
}
...
pub async fn find_user_by_credential_id(
    ...
    let user = tane_core::db_fetch_optional!(pool, User, &sql, credential_id)?;
    Ok(user)
}
...
pub async fn get_passkey_credentials(
    pool: &DbPool,
    user_id: &str,
) -> tane_core::Result<serde_json::Map<String, serde_json::Value>> {
...
pub async fn delete_passkey_from_user(
    ...
    user_id: &str,
    credential_id: &str,
) -> tane_core::Result<Option<&'static str>> {
    let method = get_auth_method(pool, user_id, "webauthn").await?;
...
pub async fn update_extra_metadata(
    ...
    let result = tane_core::db_execute!(pool, &sql, &metadata_str, user_id)?;
    Ok(result.rows_affected() > 0)
}
...
pub async fn create_api_token(
    ...
    created_by: &str,
) -> tane_core::Result<(String, String)> {
...
pub async fn get_user_api_tokens(
    pool: &DbPool,
    user_id: &str,
) -> tane_core::Result<Vec<ApiToken>> {
    let tokens = tane_core::db_fetch_all!(
        pool, ApiToken,
...
pub async fn revoke_api_token(
    ...
    let result = tane_core::db_execute!(pool, &sql, revoked_by, token_id)?;
    Ok(result.rows_affected() > 0)
}
...
pub async fn list_all_users(pool: &DbPool) -> tane_core::Result<Vec<User>> {
    let users = tane_core::db_fetch_all!(
        pool, User,
...
pub async fn admin_update_user(
    ...
    if let Some(name) = name {
        ...
        tane_core::db_execute!(pool, &sql, name, user_id)?;
    }
    ...
    if let Some(active) = active {
        ...
        tane_core::db_execute!(pool, &sql, active, user_id)?;
    }
...
pub async fn delete_user(pool: &DbPool, user_id: &str) -> tane_core::Result<bool> {
    let result = tane_core::db_execute!(
