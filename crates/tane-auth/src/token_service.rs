...
use tane_core::models::VerificationToken;
...
pub async fn verify_refresh_token(
    pool: &DbPool,
    raw_token: &str,
) -> tane_core::Result<RefreshTokenVerifyResult> {
    ...
    if let Some(replaced_at) = row.replaced_at {
        let grace_seconds = tane_core::constants::get().jwt.refresh_token_grace_period_seconds;
        ...
        if Utc::now() <= grace_deadline {
            ...
            tane_core::db_execute!(pool, &update_sql, &user_data.token_id)?;
...
pub async fn rotate_refresh_token(
    ...
    tane_core::db_execute!(pool, &replace_sql, old_token_id)?;
...
pub async fn revoke_token_family(
    ...
    let result = tane_core::db_execute!(pool, &sql, family_id)?;
...
pub async fn revoke_refresh_token(
    ...
    let result = tane_core::db_execute!(pool, &sql, token_id)?;
...
pub async fn revoke_user_refresh_token(
    ...
    let row = tane_core::db_fetch_optional!(pool, FamilyIdRow, &sql, token_id, user_id)?;
...
pub async fn get_user_sessions(
    pool: &DbPool,
    user_id: &str,
) -> tane_core::Result<Vec<SessionInfo>> {
    let is_pg = pool.is_postgres();
    let now = sql_compat::now(is_pg);
    let bt = sql_compat::bool_true(is_pg);
    ...
    let sessions = tane_core::db_fetch_all!(pool, SessionInfo, &sql, user_id)?;
    Ok(sessions)
}
...
pub async fn create_verification_token_with_expiry(
    ...
    let token_hash = bcrypt::hash(&raw_token, bcrypt::DEFAULT_COST)
        .map_err(|e| tane_core::Error::Internal(format!("bcrypt hash failed: {e}")))?;

    let id_bytes: [u8; 16] = rand::rng().random();
    let token_id = format!("vt_{}", URL_SAFE_NO_PAD.encode(id_bytes));

    let default_hours = tane_core::constants::get().jwt.email_verification_expire_hours as f64;
...
pub async fn verify_verification_token(
    ...
    raw_token: &str,
    token_type: &str,
) -> tane_core::Result<Option<String>> {
    let is_pg = pool.is_postgres();
    let now = sql_compat::now(is_pg);
    let bf = sql_compat::bool_false(is_pg);
    let bt = sql_compat::bool_true(is_pg);
    ...
    let tokens = tane_core::db_fetch_all!(pool, VerificationToken, &fetch_sql, token_type)?;
    ...
    for db_token in tokens {
        ...
        if is_match {
            ...
            tane_core::db_execute!(pool, &update_sql, &db_token.token_id)?;
