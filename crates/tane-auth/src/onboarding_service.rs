...
use tane_core::{Config, DbPool, KVPool};
...
pub async fn accept_terms(
...
) -> tane_core::Result<AcceptTermsOutcome> {
    ...
    if let Some(signup_data) = redis_ops::get_pending_signup(kv, temp_token).await? {
        let email = signup_data["email"]
            .as_str()
            .ok_or_else(|| tane_core::Error::Internal("Missing email in signup data".into()))?;
    ...
    if let Some(terms_data) = redis_ops::get_pending_terms(kv, temp_token).await? {
        let user_id = terms_data["user_id"]
            .as_str()
            .ok_or_else(|| tane_core::Error::Internal("Missing user_id in terms data".into()))?;
