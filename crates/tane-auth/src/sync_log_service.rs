...
use tane_core::{db_execute, db_fetch_all, db_fetch_scalar, DbPool};
use tane_types::sync::{SyncAction, SyncActionType};
...
impl SyncLogRow {
    fn into_sync_action(self) -> tane_core::Result<SyncAction> {
        ...
        let data = self
            ...
            .transpose()
            .map_err(|e| {
                tane_core::Error::Internal(format!("failed to parse sync_log data JSON: {e}"))
            })?;
...
fn parse_action_type(s: &str) -> tane_core::Result<SyncActionType> {
    match s {
        "insert" => Ok(SyncActionType::Insert),
        "update" => Ok(SyncActionType::Update),
        "delete" => Ok(SyncActionType::Delete),
        other => Err(tane_core::Error::Internal(format!(
...
pub async fn write_sync_entry(
    ...
    let data_str: Option<String> = data
        ...
        .transpose()
        .map_err(|e| {
            tane_core::Error::Internal(format!("failed to serialise sync entry data: {e}"))
        })?;
    ...
    let sync_id: i64 = if is_pg {
        ...
        db_fetch_scalar!(db, i64, &sql, entity_type, entity_id, workspace_id, action_str, data_str)
            .map_err(|e| {
                tane_core::Error::Internal(format!("failed to write sync entry: {e}"))
            })?
    } else {
...
pub async fn get_entries_since(
    ...
    since_sync_id: i64,
    limit: i64,
) -> tane_core::Result<Vec<SyncAction>> {
    ...
    .map_err(|e| tane_core::Error::Internal(format!("failed to get sync entries: {e}")))?;
...
pub async fn get_latest_sync_id(
    ...
    .map_err(|e| {
        tane_core::Error::Internal(format!("failed to get latest sync_id: {e}"))
    })?;
...
pub async fn is_sync_id_available(
    ...
    .map_err(|e| {
        tane_core::Error::Internal(format!("failed to check sync_id availability: {e}"))
    })?;
...
pub async fn prune_old_entries(
    ...
    let result = db_execute!(db, &sql, retention_days)
        .map_err(|e| {
            tane_core::Error::Internal(format!("failed to prune sync log entries: {e}"))
        })?;
