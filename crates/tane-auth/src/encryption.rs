...
pub fn encrypt(plaintext: &str, key: &[u8; 32]) -> tane_core::Result<String> {
    let cipher = Aes256Gcm::new(key.into());
    let nonce = Aes256Gcm::generate_nonce(&mut OsRng);

    let ciphertext_and_tag = cipher
        .encrypt(&nonce, plaintext.as_bytes())
        .map_err(|_| tane_core::Error::Internal("encryption failed".into()))?;
...
pub fn decrypt(encrypted: &str, key: &[u8; 32]) -> tane_core::Result<String> {
    let data = URL_SAFE
        .decode(encrypted)
        .map_err(|e| tane_core::Error::BadRequest(format!("bad base64: {e}")))?;

    if data.len() < 1 + NONCE_SIZE + 16 {
        return Err(tane_core::Error::BadRequest("encrypted data too short".into()));
    }
    ...
    let plaintext = cipher
        ...
        .map_err(|_| tane_core::Error::Unauthorized("decryption failed".into()))?;

    String::from_utf8(plaintext)
        .map_err(|_| tane_core::Error::Internal("decrypted data is not valid text".into()))
}

/// Encrypt a JSON value: serialize to string, then AES-256-GCM encrypt.
pub fn encrypt_json(value: &serde_json::Value, key: &[u8; 32]) -> tane_core::Result<String> {
...
pub fn decrypt_json(encrypted: &str, key: &[u8; 32]) -> tane_core::Result<serde_json::Value> {
    let json_str = decrypt(encrypted, key)?;
    serde_json::from_str(&json_str)
        .map_err(|e| tane_core::Error::Internal(format!("JSON deserialization failed: {e}")))
}

/// Derive the 32-byte encryption key from the base64url-encoded env var.
pub fn derive_key(encryption_key_b64: &str) -> tane_core::Result<[u8; 32]> {
    let bytes = URL_SAFE
        .decode(encryption_key_b64)
        .map_err(|e| tane_core::Error::Internal(format!("bad encryption key base64: {e}")))?;

    if bytes.len() != 32 {
        return Err(tane_core::Error::Internal(format!(
...
/// JSONB fields.
pub fn decrypt_slack_token(encrypted: &str, encryption_key: &[u8; 32]) -> tane_core::Result<String> {
...
pub fn encrypt_slack_token(plaintext: &str, encryption_key: &[u8; 32]) -> tane_core::Result<String> {
