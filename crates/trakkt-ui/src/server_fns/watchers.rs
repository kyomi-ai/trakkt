...
/// Check whether the current user is watching a specific issue (by number).
#[server(prefix = "/leptos-api")]
pub async fn is_watching(issue_number: i32) -> Result<bool, ServerFnError> {
    let ac = AuthenticatedContext::extract().await?;
...
/// Start watching an issue (by number).
#[server(prefix = "/leptos-api")]
pub async fn watch_issue(issue_number: i32) -> Result<(), ServerFnError> {
    let ac = AuthenticatedContext::extract().await?;
    let issue_id = super::issues::resolve_issue_id(ac.db(), &ac.ws_id, issue_number).await?;
...
/// Stop watching an issue (by number).
#[server(prefix = "/leptos-api")]
pub async fn unwatch_issue(issue_number: i32) -> Result<(), ServerFnError> {
    let ac = AuthenticatedContext::extract().await?;
    let issue_id = super::issues::resolve_issue_id(ac.db(), &ac.ws_id, issue_number).await?;
