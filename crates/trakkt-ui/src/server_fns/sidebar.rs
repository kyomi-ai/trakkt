...
pub async fn list_user_workspaces() -> Result<Vec<UserWorkspace>, ServerFnError> {
    ...
    let workspaces = trakkt_auth::workspace_service::get_user_workspaces(&ctx.db, &auth.user_id)
        .await
        .into_sfn()?;
...
pub async fn switch_workspace(workspace_id: String) -> Result<(), ServerFnError> {
    ...
    trakkt_auth::user_service::update_last_workspace(&ctx.db, &auth.user_id, &workspace_id)
        .await
        .into_sfn()?;
