...
///
/// Delegates all orchestration to `trakkt_auth::auth_service::login_with_password_service`.
#[server(prefix = "/leptos-api")]
pub async fn login_with_password(
    ...
    use trakkt_auth::auth_service::{login_with_password_service, LoginServiceResult};
    ...
    let result = login_with_password_service(trakkt_auth::auth_service::LoginWithPasswordParams {
        db: &afc.ctx.db,
        kv: &afc.kv,
...
///
/// Delegates all orchestration to `trakkt_auth::auth_service::signup_start_service`.
#[server(prefix = "/leptos-api")]
pub async fn signup_start(
    ...
    use trakkt_auth::auth_service::{signup_start_service, SignupStartServiceResult};
    ...
    let result = signup_start_service(trakkt_auth::auth_service::SignupStartParams {
        db: &afc.ctx.db,
        kv: &afc.kv,
        jwt_secret: &afc.ctx.config.jwt_secret,
        email,
...
///
/// Delegates all orchestration to `trakkt_auth::auth_service::signup_complete_service`.
#[server(prefix = "/leptos-api")]
pub async fn signup_complete(
    ...
    use trakkt_auth::auth_service::{signup_complete_service, SignupCompleteServiceResult};

    let afc = AuthFlowContext::extract().await?;

    let result = signup_complete_service(trakkt_auth::auth_service::SignupCompleteParams {
        db: &afc.ctx.db,
        kv: &afc.kv,
        jwt_secret: &afc.ctx.config.jwt_secret,
        token: &token,
...
///
/// Delegates all orchestration to `trakkt_auth::auth_service::google_oauth_callback_service`.
#[server(prefix = "/leptos-api")]
pub async fn google_oauth_callback(
    code: String,
    state: Option<String>,
) -> Result<GoogleCallbackResult, ServerFnError> {
    use trakkt_auth::auth_service::{google_oauth_callback_service, GoogleOAuthServiceResult};
    ...
    let result = google_oauth_callback_service(trakkt_auth::auth_service::GoogleOAuthCallbackParams {
        db: &afc.ctx.db,
        kv: &afc.kv,
        jwt_secret: &afc.ctx.config.jwt_secret,
        code: &code,
...
pub async fn resend_verification(email: String) -> Result<(), ServerFnError> {
    ...
    let result = trakkt_auth::auth_service::resend_verification_service(
...
pub async fn recovery_start(email: String) -> Result<(), ServerFnError> {
    ...
    let result = trakkt_auth::auth_service::recovery_start_service(
        &afc.ctx.db, &afc.kv, &afc.ip, email,
    )
    .await
    .into_sfn()?;
...
///
/// Delegates all orchestration to `trakkt_auth::auth_service::recovery_verify_service`.
#[server(prefix = "/leptos-api")]
pub async fn recovery_verify(
    token: String,
) -> Result<RecoveryVerifyResult, ServerFnError> {
    use trakkt_auth::auth_service::{recovery_verify_service, RecoveryVerifyServiceResult};
...
///
/// Delegates all orchestration to `trakkt_auth::auth_service::recovery_set_password_service`.
#[server(prefix = "/leptos-api")]
pub async fn recovery_set_password(
    recovery_session_id: String,
    new_password: String,
) -> Result<RecoverySetPasswordResult, ServerFnError> {
    use trakkt_auth::auth_service::{
...
pub async fn passkey_login_start() -> Result<PasskeyLoginStartResult, ServerFnError> {
    ...
    let (mut rcr, disc_state) =
        trakkt_auth::webauthn::start_discoverable_authentication(webauthn)
            .into_sfn()?;

    // Remove mediation hint — we want a modal prompt, not conditional UI autofill
    rcr.mediation = None;

    let challenge_id = trakkt_auth::redis_ops::generate_token();
    ...
    trakkt_auth::redis_ops::store_webauthn_challenge(&kv, &challenge_id, &challenge_data)
        .await
        .into_sfn()?;
...
///
/// Delegates all orchestration to `trakkt_auth::auth_service::passkey_login_complete_service`.
#[server(prefix = "/leptos-api")]
pub async fn passkey_login_complete(
    challenge_id: String,
    assertion_json: String,
) -> Result<LoginResult, ServerFnError> {
    use trakkt_auth::auth_service::{passkey_login_complete_service, PasskeyLoginServiceResult};

    let afc = AuthFlowContext::extract().await?;
    let webauthn = afc.ctx.webauthn()?;

    let result = passkey_login_complete_service(trakkt_auth::auth_service::PasskeyLoginCompleteParams {
        db: &afc.ctx.db,
        kv: &afc.kv,
        jwt_secret: &afc.ctx.config.jwt_secret,
        webauthn,
...
///
/// Delegates all orchestration to `trakkt_auth::auth_service::passkey_register_start_service`.
...
pub async fn passkey_register_start(
    ...
    let result = passkey_register_start_service(trakkt_auth::auth_service::PasskeyRegisterStartParams {
        db: &afc.ctx.db,
        kv: &afc.kv,
        webauthn,
        email: email_trimmed,
...
///
/// Delegates all orchestration to `trakkt_auth::auth_service::passkey_register_complete_service`.
#[server(prefix = "/leptos-api")]
pub async fn passkey_register_complete(
    ...
    let sess = trakkt_auth::auth_service::passkey_register_complete_service(
        &afc.ctx.db,
        &afc.kv,
        &afc.ctx.config.jwt_secret,
        webauthn,
...
///
/// Delegates all orchestration to `trakkt_auth::auth_service::passkey_signup_complete_service`.
#[server(prefix = "/leptos-api")]
pub async fn passkey_signup_complete(
    ...
    let result = trakkt_auth::auth_service::passkey_signup_complete_service(
        trakkt_auth::auth_service::PasskeySignupCompleteParams {
            db: &ctx.db,
            kv: &kv,
            webauthn,
            token: &token,
...
///
/// Delegates all orchestration to `trakkt_auth::auth_service::passkey_recovery_verify_service`.
#[server(prefix = "/leptos-api")]
pub async fn passkey_recovery_verify(
    ...
    let result = trakkt_auth::auth_service::passkey_recovery_verify_service(
        &ctx.db,
        &kv,
        webauthn,
        &token,
...
/// service layer.
#[cfg(feature = "ssr")]
fn set_session_cookies(sess: &trakkt_auth::session::AuthenticatedSession) {
...
#[cfg(feature = "ssr")]
pub(crate) fn extract_device_info(headers: &axum::http::HeaderMap) -> trakkt_auth::token_service::DeviceInfo {
    ...
    trakkt_auth::token_service::DeviceInfo {
        user_agent,
