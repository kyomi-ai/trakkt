...
pub async fn has_password() -> Result<bool, ServerFnError> {
    ...
    trakkt_auth::user_service::has_password(&ctx.db, &auth.user_id)
        .await
        .into_sfn()
}
...
pub async fn set_password(new_password: String) -> Result<String, ServerFnError> {
    ...
    let has_pw = trakkt_auth::user_service::has_password(&ctx.db, &auth.user_id)
        .await
        .into_sfn()?;
    if has_pw {
        return Err(ServerFnError::new(
    ...
    // Hash and store
    let hash = trakkt_auth::password::hash_password(&new_password)
        .into_sfn()?;
    let auth_data = serde_json::json!({"hash": hash});
    trakkt_auth::user_service::upsert_auth_method(&ctx.db, &auth.user_id, "password", &auth_data)
        .await
        .into_sfn()?;
...
pub async fn change_password(
    ...
    trakkt_auth::security_service::change_password(
        &ctx.db,
        &auth.user_id,
        &current_password,
        &new_password,
...
pub async fn get_totp_status() -> Result<TotpStatus, ServerFnError> {
    ...
    let enabled = trakkt_auth::user_service::has_totp_enabled(&ctx.db, &auth.user_id)
        .await
        .into_sfn()?;
...
pub async fn setup_totp() -> Result<TotpSetup, ServerFnError> {
    ...
    let result =
        trakkt_auth::security_service::setup_totp(&ctx.db, &kv, &auth.user_id, &auth.email)
            .await
            .into_sfn()?;
...
pub async fn enable_totp(code: String) -> Result<String, ServerFnError> {
    ...
    trakkt_auth::security_service::enable_totp(&ctx.db, &kv, &auth.user_id, &code)
        .await
        .into_sfn()?;
...
pub async fn disable_totp() -> Result<String, ServerFnError> {
    ...
    trakkt_auth::user_service::remove_auth_method(&ctx.db, &auth.user_id, "totp")
        .await
        .into_sfn()?;
...
///
/// Maps to the `SessionInfo` fields from `trakkt_auth::token_service` plus
...
pub async fn get_sessions() -> Result<Vec<SessionEntry>, ServerFnError> {
    ...
    let refresh_token_name = &trakkt_core::constants::get().cookies.refresh_token_name;
    let raw_token = trakkt_auth::cookies::get_cookie_value(&headers, refresh_token_name);

    let (sessions, current_family_id) =
        trakkt_auth::security_service::get_sessions(&ctx.db, &auth.user_id, raw_token)
            .await
            .into_sfn()?;
...
pub async fn revoke_session(token_id: String) -> Result<String, ServerFnError> {
    ...
    let revoked =
        trakkt_auth::token_service::revoke_user_refresh_token(&ctx.db, &auth.user_id, &token_id)
            .await
            .into_sfn()?;
...
pub async fn logout() -> Result<(), ServerFnError> {
    ...
    trakkt_auth::security_service::logout(&ctx.db, raw_token)
        .await
        .into_sfn()?;
    ...
    trakkt_auth::cookies::clear_token_cookies(&mut cookie_headers);
...
pub async fn logout_all_sessions() -> Result<String, ServerFnError> {
    ...
    let revoked_count =
        trakkt_auth::token_service::revoke_all_user_refresh_tokens(&ctx.db, &auth.user_id)
            .await
            .into_sfn()?;
...
pub async fn list_passkeys() -> Result<Vec<PasskeyInfo>, ServerFnError> {
    ...
    let creds = trakkt_auth::user_service::get_passkey_credentials(&ctx.db, &auth.user_id)
        .await
        .into_sfn()?;
...
pub async fn start_passkey_registration(
    ...
    let (ccr, challenge_id) = trakkt_auth::security_service::start_passkey_registration(
        &ctx.db,
        &kv,
        webauthn,
        &auth.user_id,
...
pub async fn complete_passkey_registration(
    ...
    let device_name = trakkt_auth::security_service::complete_passkey_registration(
        &ctx.db,
        &kv,
        webauthn,
        &auth.user_id,
...
pub async fn delete_passkey(credential_id: String) -> Result<String, ServerFnError> {
    ...
    match trakkt_auth::user_service::delete_passkey_from_user(
        &ctx.db,
        &auth.user_id,
        &credential_id,
    )
...
pub async fn rename_passkey(
    ...
    let updated = trakkt_auth::user_service::update_passkey_device_name(
        &ctx.db,
        &auth.user_id,
        &credential_id,
        &trimmed,
