...
// On the server, re-export the canonical definition from `trakkt-auth`.
...
// `trakkt-auth` is not compiled for the WASM target.
#[cfg(feature = "ssr")]
pub use trakkt_auth::workspace_service::OwnershipTransferDetail;

/// Client-side mirror of `trakkt_auth::workspace_service::OwnershipTransferDetail`.
...
pub async fn get_ownership_transfer(
    ...
    trakkt_auth::workspace_service::get_transfer_for_recipient(
        &ctx.db,
        &transfer_id,
        &auth.user_id,
    )
...
pub async fn accept_ownership_transfer(
    ...
    let transfer = trakkt_auth::workspace_service::get_ownership_transfer(&ctx.db, &transfer_id)
        .await
        .into_sfn()?
    ...
    if transfer.expires_at < chrono::Utc::now() {
        let _ = trakkt_auth::workspace_service::update_transfer_status(
            &ctx.db,
            &transfer_id,
            "expired",
        )
    ...
    trakkt_auth::workspace_service::complete_ownership_transfer(
        &ctx.db,
        &transfer_id,
...
pub async fn decline_ownership_transfer(
    ...
    trakkt_auth::workspace_service::update_transfer_status(&ctx.db, &transfer_id, "declined")
        .await
        .into_sfn()?;
