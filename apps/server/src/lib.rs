...
//! tane-server — Axum HTTP server for the Tane backend.
...
use tane_core::db::DbPool;
use tane_core::sql_compat;
use tane_core::{db_execute, db_fetch_scalar};

/// Auto-provision a local user and workspace for personal (desktop) mode.
pub async fn auto_provision_personal_mode(db: &DbPool) -> Result<(), tane_core::Error> {
...
pub fn build_router(state: state::AppState) -> Router {
    ...
    Router::new()
        ...
        .route("/leptos-api/{*fn_name}", axum::routing::post({
            let server_ctx = tane_ui::server_fns::ServerContext {
                db: state.db.clone(),
                config: state.config.clone(),
                auth_state: tane_auth::middleware::AuthState::from_ref(&state),
