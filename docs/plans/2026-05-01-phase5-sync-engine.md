...
- Kyomi sync engine: `crates/kyomi-auth/src/websocket/`, `sync_log_service.rs`
- Kyomi client cache: `crates/kyomi-ui/src/cache/`
- Kyomi WS route: `apps/server/src/routes/websocket.rs`
...
2. `crates/trakkt-auth/src/websocket/manager.rs` — WebSocketManager:
   - Copy from `crates/kyomi-auth/src/websocket/manager.rs`
...
2. `apps/server/src/routes/ws.rs` — WebSocket handler:
   - Copy pattern from `apps/server/src/routes/websocket.rs`
...
2. `db.rs` — IndexedDB operations:
   - Copy pattern from `crates/kyomi-ui/src/cache/db.rs`
...
1. `crates/trakkt-ui/src/cache/store.rs` — SyncStore:
   - Copy pattern from `crates/kyomi-ui/src/cache/store.rs`
...
2. `crates/trakkt-ui/src/cache/sync_engine.rs` — Sync engine:
   - Copy pattern from `crates/kyomi-ui/src/cache/sync_engine.rs`
